\begin{figure}
    \textbf{Subtyping}\\
    \infax[Base]
    {\subtyp{\iota}{\iota}}

    \infrule[Null-Sup]
    {\subtyp{D}{E}}
    {\subtyp{D}{E?}}

    \infrule[Null]
    {\subtyp{D}{E}}
    {\subtyp{D?}{E?}}

    \infrule[Arrow]
    {\subtyp{A'}{A} \andalso \subtyp{B}{B'}}
    {\subtyp{\arrow{A}{B}}{\arrow{A'}{B'}}}

    \textbf{Naive Subtyping}\\
    \infax[Naive-Base]
    {\subtypn{\iota}{\iota}}

    \infrule[Naive-Null-Sup]
    {\subtypn{D}{E}}
    {\subtypn{D}{E?}}

    \infrule[Naive-Null]
    {\subtypn{D}{E}}
    {\subtypn{D?}{E?}}

    \infrule[Naive-Arrow]
    {\subtypn{A}{A'} \andalso \subtypn{B}{B'}}
    {\subtypn{\arrow{A}{B}}{\arrow{A'}{B'}}}

    \textbf{Positive and Negative Subtyping}\\
    \infax[Positive-Base]
    {\subtyppos{\iota}{\iota}}

    \infrule[Positive-Null-Sup]
    {\subtyppos{D}{E}}
    {\subtyppos{D}{E?}}

    \infrule[Positive-Null]
    {\subtyppos{D}{E}}
    {\subtyppos{D?}{E?}}

    \infrule[Positive-Arrow]
    {\subtypneg{A'}{A} \andalso \subtyppos{B}{B'}}
    {\subtyppos{\arrow{A}{B}}{\arrow{A'}{B'}}}

    \infax[Negative-Base]
    {\subtypneg{\iota}{\iota}}

    \infrule[Negative-Null-Sup]
    {\subtypneg{D}{E}}
    {\subtypneg{D}{E?}}

    \infrule[Negative-Null]
    {\subtypneg{D}{E}}
    {\subtypneg{D?}{E?}}

    \infrule[Negative-Arrow]
    {\subtyppos{A'}{A} \andalso \subtypneg{B}{B'}}
    {\subtypneg{\arrow{A}{B}}{\arrow{A'}{B'}}}

  \caption{Subtyping rules of the explicit language}\label{fig:subtyping}
\end{figure}
