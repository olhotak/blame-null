...
\begin{itemize}
    ...
    \item A term $s$ of type $!(S\to T)$ can be applied using either standard application $\app{s}{t}$ or safe application $\safeapp{s}{t}{u}$.
\end{itemize}
...
On top of $\lambdanull$, \citeauthor{DBLP:conf/ecoop/NietoRRL19} define $\lambdanulls$, a calculus representing
...
It is thus possible to call functions of one language from the
other language. 
...
these casts always blames the implicit language. We survey related work
in \Cref{sec:rw} and conclude in \Cref{sec:conclusion}.

We have formalized all of our lemmas and theorems in Coq. We will
submit our Coq formalization to the OOPSLA Artifact Evaluation process.
