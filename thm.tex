...
defined for the calculus in \Cref{fig:subtyping}. Intuitively,
a cast between types related by positive subtyping cannot give rise to positive
blame and a cast between types related by negative subtyping cannot give rise to
negative blame. Ordinary subtyping is then an intersection of these two relations,
so a cast between types related by ordinary subtyping cannot give rise to any blame.
We will discuss naive subtyping and its relationship to the other three subtyping relations
in \Cref{sec:tangram}.
