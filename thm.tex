\section{Properties of the Explicit Language\label{sec:thm}}

\subsection{Type Safety}

We have proven type safety of the explicit language following the syntactic approach of \citet{DBLP:journals/iandc/WrightF94}.

\begin{theorem}[Preservation\label{thm:preservation}] \coq{preservation}\\
...
\begin{lemma}[Substitution] \coq{substitution}\\
...
\begin{lemma}[Compatibility Symmetry] \coq{compat\_sym}
...
\begin{theorem}[Progress] \coq{progress} \\
...
\begin{lemma}[Canonical Forms Arrow] \coq{canonical\_forms\_arrow}\\
...
the approach of \citet{Wadler-and-Findler} (see also \citet{DBLP:conf/snapl/Wadler15} for a more accessible summary of the approach). The applicability of this standard
approach is one of the benefits of the explicit language relative to the calculus of
\citet{Nieto-et-al-2019}.

The approach depends on four subtyping relations, defined for the
explicit language in \Cref{fig:subtyping}.  Intuitively, a cast between types
...
(blame with a label that is the complement of that on the
...
With this definition, we can prove blame safety of the explicit language, that
when $\sfor{N}{p}$, $N$ cannot reduce to $\blame{p}$ in any number of
steps.

\begin{theorem}[Safe Term Preservation\label{thm:sfor-preservation}] \coq{sfor\_preservation}\\
...
\begin{lemma}[Substitution Preserves Safe Terms] \coq{subst\_pres\_sfor}\\
...
\begin{corollary}[Safe Term Progress\label{thm:sfor-progress}]
    \coq{sfor\_progress}\\
    If $\typing{}{N}{A}$, $\sfor{N}{p}$, and $\red{N}{\blame{q}}$, then $p \neq q$.
\end{corollary}
\begin{proof}
    This follows directly from \Cref{thm:sfor-preservation} and the definition of \textsf{safe}.
\end{proof}

\begin{theorem}[Blame Safety] \coq{safety}\\
    If $\typing{}{N}{A}$, $\sfor{N}{p}$, and $\redst{N}{\blame{q}}$, then $p \neq q$.
\end{theorem}
\begin{proof}
    The proof is by induction on the transitive reduction relation. In the
    inductive case, it uses \Cref{thm:preservation,thm:sfor-preservation}.
\end{proof}
...
The Tangram Lemma of \citet{Wadler-and-Findler} relates these four subtyping
relations. We show that it holds for the specific relations defined in \Cref{fig:subtyping}.

\begin{theorem}[Tangram Lemma\label{thm:tangram}] \coq{tangram\_fwd}
    \coq{tangram\_rev}
...
% OL: I've removed the shading. It did not have any semantic meaning other than to draw attention to certain details.
